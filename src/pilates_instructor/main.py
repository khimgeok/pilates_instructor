...
def train():
    ...
    try:
        PilatesInstructor().crew().train(n_iterations=int(sys.argv[1]), filename=sys.argv[2], inputs=inputs)
...
def test():
    ...
    try:
        PilatesInstructor().crew().test(n_iterations=int(sys.argv[1]), eval_llm=sys.argv[2], inputs=inputs)
